...
class Companyinfo{
  ...
  late bool active;
  Companyinfo({required this.companyname,required this.company_phonenumber,required this.active});
    ...
    UserT.get().then((value)  {
      value.docs.forEach((element) async{
        ...
        await UserT.where('email', isEqualTo: element.id.toString()).get().then((value) => value.docs.forEach((element) {
            ...
            list.add(Companyinfo(companyname: companyname, company_phonenumber: company_phonenumber,active: active));
            streamController.add(Companyinfo(companyname: companyname, company_phonenumber: company_phonenumber,active: active));
                          ...
                          : Expanded(
                            child: ListView.separated(
                              ...
                              itemBuilder:(BuildContext context, int index){
                                return NoOfReques(website: website,companyname: list[index].companyname,phonenumber:list[index].company_phonenumber, active: list[index].active,email: email,);
                            }
                           ),
                          );
...
class NoOfReques extends StatelessWidget {
  NoOfReques({
    ...
    Key? key, required this.phonenumber, required this.active, required this.email, required this.website,
  }) : super(key: key);
    ...
    return Padding(
      ...
      child: GestureDetector(
        onTap: (){
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => SelectedCompany(website: website,companyname: companyname,phonenumber: phonenumber,active: active,email: email,)),
          );
        },
        child: Container(
          height: 55.h,
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              onTap: (){
                UserT.doc(widget.email).update({"company_name":"${textEditingController1.text.toString()}","phone_number":"${textEditingController2.text.toString()}","website":"${textEditingController3.text.toString()}"});
              ...
              child: Container(
                  ...
                  child:Center(
                    child: Text("Save",style: GoogleFonts.poppins(fontSize: 14.sp,color: shapeitemColor(context),fontWeight: FontWeight.w500),),
                  )
                ),
              ),
          ],
